import { Divergencia, TaxCompliance, AuditRecipe, AuditRule, MaterialMovement, StockPosition } from '../types/audit';
import { safeLocalStorageSet, setLargeData } from './storageUtils';

export const CalcularRelatorioAuditoria = (
  allMovements: MaterialMovement[],
  estoqueInicial: StockPosition[],
  estoqueFinal: StockPosition[]
) => {
    // 1. Get all unique materials
    const materialsSet = new Set([
        ...allMovements.map(m => String(m.material).trim()),
        ...estoqueInicial.map(s => String(s.material).trim()),
        ...estoqueFinal.map(s => String(s.material).trim())
    ]);

    // 2. Filter out centers and totals
    const isMaterialValid = (mat: string) => {
        if (!mat) return false;
        const m = mat.trim();
        if (m === '1001' || m === '1005' || m === 'Total Geral' || m.endsWith('Total')) return false;
        return true;
    };
    
    // 2. Perform aggregation
    return Array.from(materialsSet).filter(isMaterialValid).map(material => {
        const movs = allMovements.filter(m => String(m.material).trim() === material);
        const inicial = estoqueInicial.find(s => String(s.material).trim() === material)?.quantity || 0;
        const final = estoqueFinal.find(s => String(s.material).trim() === material)?.quantity || 0;
        
        const totals: Record<string, number> = {
            'Produção/Compras': 0, 'Venda': 0, 'Devolução Entrada': 0,
            'Devolução Compras': 0, 'Bonificação': 0, 'Outras Saídas': 0,
            'Perdas': 0, 'Ajuste de Saída': 0, 'Ajuste de Entrada': 0, 'Requisição': 0
        };

        movs.forEach(mov => {
            const cat = mov.category || 'Outros';
            if (totals.hasOwnProperty(cat)) {
                totals[cat] += mov.quantity;
            }
        });

        // HTML logic categorization
        const somaEntradas = totals['Produção/Compras'] + totals['Devolução Entrada'] + totals['Ajuste de Entrada'] + totals['Bonificação'];
        const somaSaidas = totals['Venda'] + totals['Devolução Compras'] + totals['Outras Saídas'] + totals['Perdas'] + totals['Ajuste de Saída'] + totals['Requisição'];

        return {
            material,
            inicial,
            finalStockReal: final,
            ...totals,
            somaEntradas,
            somaSaidas,
            movimentacaoDoMes: somaEntradas + somaSaidas,
            diferenca: (inicial + somaEntradas - somaSaidas) - final // This is the audit difference
        };
    });
};
